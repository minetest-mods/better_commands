        ...
        elseif action == "query" then
            if time == "daytime" then
                if better_commands.settings.acovg_time then
                    return true, S("Current time: @1", math.floor(core.get_timeofday()*24000+18000) % 24000), 1
                else
                    return true, S("Current time: @1", math.floor(core.get_timeofday()*24000)), 1
                end
            elseif time == "gametime" then
                return true, S("Time since world creation: @1", (core.get_gametime() or 0)*24000), 1
            elseif time == "day" then
                return true, S("Day count: @1", core.get_day_count()), 1
            end
