...
better_commands.register_command("say", {
    params = S("<message>"),
    description = S(
    "Broadcasts <message> to all players (<message> can include selectors such as @@a if you have the 'server' priv)"),
    privs = { shout = true },
...
better_commands.register_command("msg", {
    ...
    description = S(
    "Sends <message> privately to <target> (<message> can include selectors like @@a if you have the 'server' priv)"),
    privs = { shout = true },
    func = function(name, param, context)
        local split_param = better_commands.parse_params(param)
...
better_commands.register_command("me", {
    params = S("<action>"),
    description = S(
    "Broadcasts a message about yourself (<message> can include selectors like @@a if you have the 'server' priv)"),
    privs = { shout = true },
    func = function(name, param, context)
        local split_param = better_commands.parse_params(param)
        if not split_param[1] then return false, nil, 0 end
...
better_commands.register_command("teammsg", {
    ...
    func = function(name, param, context)
        ...
        local start = S("[@1] <@2> ", core.colorize(team_color, display_name),
            better_commands.get_entity_name(context.executor))
        local message
        if context.command_block or core.check_player_privs(context.origin, { server = true }) then
            local err
            message, err = better_commands.expand_selectors(param, split_param, 1, context)
