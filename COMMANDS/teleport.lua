    ...
    func = function(name, param, context)
        ...
        if split_param[1].type == "selector" then
            if not split_param[2] then
                if not context.executor.is_player then
                    return false,
                        better_commands.error(S(
                            "Command blocks can't teleport (although I did consider making it possible)")), 0
                end
                ...
                return true,
                    S("Teleported @1 to @2", better_commands.get_entity_name(context.executor),
                        better_commands.get_entity_name(targets[1])), 1
        ...
        elseif split_param[1].type == "number" or split_param[1].type == "relative" or split_param[1].type == "look_relative" then
            if not context.executor.is_player and split_param[1][3] == "@s" then
                return false,
                    better_commands.error(S("Command blocks can't teleport (although I did consider making it possible)")),
                    0
            end
            ...
            return true,
                S("Teleported @1 to @2", better_commands.get_entity_name(context.executor),
                    core.pos_to_string(target_pos, 1)), 1
        end
        return false, nil, 0
    end
})
