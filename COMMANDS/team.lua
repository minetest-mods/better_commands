    ...
    func = function(name, param, context)
        ...
        if subcommand == "add" then
            ...
            if team_name:find("[^%w_]") then
                return false,
                    better_commands.error(S("Invalid team name @1: Can only contain letters, numbers, and underscores",
                        team_name)), 0
            end
        ...
        elseif subcommand == "join" then
            ...
            if not selector then
                ...
                elseif count == 1 then
                    return true,
                        S("Added @1 to team [@2]", better_commands.format_name(last),
                            better_commands.format_team_name(team_name)), 1
                else
        ...
        elseif subcommand == "leave" then
            ...
            elseif count == 1 then
                ...
                return true, S("Removed @1 from any team", tostring(count)), 1
            end
        ...
        elseif subcommand == "modify" then
            ...
            if key == "color" then
                if value then
                    if not better_commands.team_colors[value] then
                        return false,
                            better_commands.error(S("Invalid color: @1", value)), 0
                    end
            ...
            elseif key == "displayName" then
                if value then
                    ...
                    return true,
                        S("Set display name of team [@1] to @2", better_commands.format_team_name(team_name), value), 1
                else
            ...
            elseif key == "friendlyFire" then
                ...
                return true,
                    S("Set friendly fire for team [@1] to @2", better_commands.format_team_name(team_name), value), 1
            elseif key == "nameFormat" then
                ...
                return true, S("Set name format for team [@1] to @2", better_commands.format_team_name(team_name), value),
                    1
            else
                return false,
                    better_commands.error(S("Value must be 'color', 'displayName', 'friendlyFire', or 'nameFormat'")), 0
            end
        end
        return false,
            better_commands.error(S("Must be 'add', 'empty', 'join', 'leave', 'list', 'modify', or 'remove', not @1",
                subcommand)), 0
    end
})
