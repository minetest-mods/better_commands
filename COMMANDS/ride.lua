    ...
    func = function(name, param, context)
        ...
        if action == "mount" or action == "start_riding" then
                ...
                if split_param[8] then
                    ...
                    if teleport_rules then
                        ...
                        elseif teleport_rules ~= "teleport_rider" then
                            return false,
                                better_commands.error(S("Expected teleport_ride|teleport_rider, got @1", teleport_rules)),
                                0
                        end
                    end
                end
            ...
            if not target[1].is_player then
                return false, better_commands.error(S("Command blocks cannot ride objects. This should be obvious.")), 0
            end
            ...
            if not vehicle[1].is_player then
            ...
            return true,
                S("@1 started riding @2", better_commands.get_entity_name(target[1]),
                    better_commands.get_entity_name(vehicle[1])), 1
        elseif action == "dismount" or action == "stop_riding" then
            local target, err = better_commands.parse_selector(target_selector, context, true)
            if err or not target then return false, better_commands.error(err), 0 end
            local vehicle = target[1].get_attach and target[1]:get_attach()
            if not vehicle then
                return false,
                    better_commands.error(S("@1 is not riding any vehicle", better_commands.get_entity_name(target[1]))),
                    0
            end
            target[1]:set_detach()
            return true,
                S("@1 stopped riding @2", better_commands.get_entity_name(target[1]),
                    better_commands.get_entity_name(vehicle)), 1
        end
        return false, nil, 0
    end
})
